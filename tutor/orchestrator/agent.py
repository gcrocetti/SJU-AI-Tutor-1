...
student interaction flow and determines which specialized tutor should handle each query.
It performs:

1. Query analysis to understand the student's intent
2. Agent selection based on query characteristics
3. Query reformulation for specialized tutor when needed
4. Maintenance of conversation context across multiple tutor
...
from tutor.orchestrator.prompts import SYSTEM_PROMPT, QUERY_ANALYSIS_PROMPT
from tutor.orchestrator.tools import format_agent_descriptions, format_conversation_history
...
class AgentSelection(BaseModel):
    ...
    require_aggregation: bool = Field(
        description="Whether the results from multiple tutor need to be aggregated",
        default=False
    )
...
def query_analyzer(state: OrchestratorState) -> OrchestratorState:
    """
    Analyze the user query to determine which tutor should handle it.
...
Determine which specialized tutor should handle this query.
...
""")
    ...
    try:
        ...
        logger.info(f"Query analysis selected tutor: {new_state['agent_selection'].get('selected_agents', [])}")
...
def route_to_agents(state: OrchestratorState) -> OrchestratorState:
    """
    Route the query to the selected tutor and collect their responses.
    ...
    try:
        ...
        for agent_id in selection["selected_agents"]:
            ...
            if agent_id == "university":
                ...
                from tutor.teacher_agent.agent import process_query as teacher_process
                ...
                from tutor.academic_coach_agent.agent import process_query as academic_coach_process
        ...
        logger.error(f"Error routing to tutor: {e}")
        new_state = state.copy()
        new_state["error"] = f"Failed to route to tutor: {str(e)}"
        return new_state
...
def generate_final_response(state: OrchestratorState) -> OrchestratorState:
    ...
    try:
        ...
        # If multiple responses need to be aggregated, use the aggregator agent
            ...
            from tutor.aggregator.agent import aggregate_responses
...
def process_query(query: str, session_id: str, message_history: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Determine which tutor were used
...
if __name__ == "__main__":
    ...
    print(f"Used tutor: {result.get('used_agents', [])}")
