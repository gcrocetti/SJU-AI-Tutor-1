...
import tutor.university_agent.config
...
from tutor.university_agent.config import TEMPERATURE_DETERMINISTIC, LLM_MODEL_FAST, TEMPERATURE_NORMAL
from tutor.university_agent.prompts import SYSTEM_PROMPT, QUERY_ANALYSIS_PROMPT
from tutor.university_agent.tools import google_search_query
...
# Initialize the LLM
def get_llm(temperature: float = TEMPERATURE_NORMAL) -> ChatOpenAI:
    """
...
def query_analyzer(state: AgentState) -> AgentState:
    ...
    """
    llm = get_llm(temperature=TEMPERATURE_DETERMINISTIC)  # Low temperature for more deterministic analysis
